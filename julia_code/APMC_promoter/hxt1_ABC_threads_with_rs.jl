...
function rho_lens15(d2)
    pars = copy(d2)
    fixed_p = readdlm("potential_particles/int_g_midpoint_parameters_985.txt")[:,1]
...
for ii = 6:10
    ...
    writedlm("hxt1_985/p_$(ii).txt", apmc_output.p)
    writedlm("hxt1_985/e_$(ii).txt", apmc_output.epsilon)
    for jj in 1:8
        writedlm("hxt1_985/pts_$(ii)_$jj.txt",apmc_output.pts[jj,end])
        writedlm("hxt1_985/wts_$(ii)_$jj.txt",apmc_output.wts[jj,end])
    end
end
...
for i = [1]
    for j = 1:10
        p = readdlm("hxt$(i)_985/p_$j.txt")
        eval(Meta.parse("final_p$i"))[:,j] = p[:,end]
        e = readdlm("hxt$(i)_985/e_$j.txt")
...
savefig("heatmap_final_probabilities_hxt_rejigged_$(i)_985.png")

# ii=findmin(final_e1)[2]
# win=findmax(final_p1[:,ii])[2]
# win_pts = readdlm("hxt1_985/pts_$(ii)_$win.txt")[:,end]

win=findmax(mean(final_p1,dims=2))[2][1]
is=findall(final_p1[win,:] .> 0.5)
ii = is[findmin(final_e1[is])[2]]
win_pts = readdlm("hxt1_985/pts_$(ii)_$win.txt")[:,end]
...
savefig("winning_model_$(win)_sample_solution_HXT1_985.png")
...
for j = 1:10
    p = readdlm("hxt1_985/p_$j.txt")
    eval(Meta.parse("final_p1"))[:,j] = p[:,end]
    e = readdlm("hxt1_985/e_$j.txt")
    eval(Meta.parse("final_e1"))[j] = e[end]
end
writedlm("final_p1_985.txt",final_p1)
writedlm("final_e1_985.txt",final_e1)
...
# win = win+3
# pars = readdlm("hxt1_985/pts_$(ii)_$win.txt")[:,end]
# fixed_p = readdlm("potential_particles/int_g_midpoint_parameters_985.txt")[:,1]
# pars = 10 .^ [fixed_p[1:29];pars[1:5];10.0;0.0;pars[6:7];10.0;pars[8];0.0;pars[9];fixed_p[43:48];fixed_p[49:52]]#mth1,mig1
# pars[35] = -10^10
# writedlm("hxt_1_best_particle_model_5.txt",pars)

# orig = readdlm("hxt1_985/pts_$(ii)_$win.txt")
...
#     final_pars[:,i] .= pars
# end
# writedlm("hxt_1_best_particle_model_5_posterior.txt",final_pars)
...
for ii = 1:10, win in [1,2,3,4,5,6,7,8]
    #win=findmax(final_p1[:,ii])[2]
    if final_p1[win,ii]>0.01
        if win == 2
            pars = readdlm("hxt1_985/pts_$(ii)_$win.txt")[:,1]
            fixed_p = readdlm("potential_particles/int_g_midpoint_parameters_985.txt")[:,1]
            pars = 10 .^ [fixed_p[1:29];pars[1:5];10.0;0.0;pars[6:7];10.0;pars[8];0.0;pars[9];fixed_p[43:48];fixed_p[49:52]]#mth1,mig1
            pars[35] = -10^10
        elseif win == 1 
        ...
        writedlm("best_fit_parameters_each_model_each_run_985/hxt_1_best_particle_model_$(win)_run_$(ii).txt",pars)
    end
end