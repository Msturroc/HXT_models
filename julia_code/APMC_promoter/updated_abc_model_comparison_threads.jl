...
function APMC_threaded(N, models, rho,;names=Vector[[string("parameter", i) for i in 1:length(models[m])] for m in 1:length(models)],prop=0.5,paccmin=0.01,n=2,covar="LinearShrinkage(DiagonalUnequalVariance(), :lw)",perturb="Cauchy")
    i = 1
    lm = length(models)
    s = round(Int, N * prop)
  # array for number of parameters in each model
