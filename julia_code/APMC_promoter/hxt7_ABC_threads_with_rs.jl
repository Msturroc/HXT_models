...
for ii = 1:10
    ...
    writedlm("hxt7_955/p_$(ii).txt", apmc_output.p)
    writedlm("hxt7_955/e_$(ii).txt", apmc_output.epsilon)
    for jj in 1:4
        writedlm("hxt7_955/pts_$(ii)_$jj.txt",apmc_output.pts[jj,end])
        writedlm("hxt7_955/wts_$(ii)_$jj.txt",apmc_output.wts[jj,end])
    end
end
...
# win_pts = readdlm("hxt7_955_2k/pts_$(ii)_$win.txt")[:,end]

win=findmax(mean(final_p7,dims=2))[2][1]
is=findall(final_p7[win,:] .> 0.5)
ii = findmin(final_e7[is])[2]
win_pts = readdlm("hxt7_955/pts_$(ii)_$win.txt")[:,end]
...
savefig("winning_model_$(win)_sample_solution_hxt7_955.png")
...
for j = 1:10
    p = readdlm("hxt7_955/p_$j.txt")
    eval(Meta.parse("final_p7"))[:,j] = p[:,end]
    e = readdlm("hxt7_955/e_$j.txt")
    eval(Meta.parse("final_e7"))[j] = e[end]
end
writedlm("final_p7.txt",final_p7)
writedlm("final_e7.txt",final_e7)

for ii = 1:10, win in [1,2,3,4]
    #win=findmax(final_p1[:,ii])[2]
    if final_p7[win,ii]>0
        if win==1
            pars = readdlm("hxt7_955/pts_$(ii)_$win.txt")[:,1]
            fixed_p = readdlm("potential_particles/int_g_midpoint_parameters_955.txt")[:,1]
            pars = 10 .^ [fixed_p[1:29];pars[1:9];10.0;fixed_p[40];1.0;1.0;fixed_p[43:48];fixed_p[49:52]]
            pars[42] = 1e10 #mig1, mig2
        elseif win==2
        ...
        writedlm("best_fit_parameters_each_model_each_run/hxt_7_best_particle_model_$(win)_run_$(ii).txt",pars)
    end
end