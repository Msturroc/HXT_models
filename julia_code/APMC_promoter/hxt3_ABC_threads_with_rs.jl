...
for ii = 1:10
    ...
    writedlm("hxt3_985/e_$(ii).txt", apmc_output.epsilon)
    for jj in 1:8
        writedlm("hxt3_985/pts_$(ii)_$jj.txt",apmc_output.pts[jj,end])
        writedlm("hxt3_985/wts_$(ii)_$jj.txt",apmc_output.wts[jj,end])
    end
end
...
win=findmax(mean(final_p3,dims=2))[2][1]
is=findall(final_p3[win,:] .> 0.5)
ii = findmin(final_e3[is])[2]
win_pts = readdlm("hxt3_985/pts_$(ii)_$win.txt")[:,end]
...
savefig("winning_model_$(win)_sample_solution_hxt3_985.png")

# bar(apmc_output.p[:,end],xlabel="model",ylabel="final probability")
# savefig("final_model_probabilities_hxt3.png")
...
for j = 1:10
    p = readdlm("hxt3_985/p_$j.txt")
    eval(Meta.parse("final_p3"))[:,j] = p[:,end]
    e = readdlm("hxt3_985/e_$j.txt")
    eval(Meta.parse("final_e3"))[j] = e[end]
end
writedlm("final_p3_985.txt",final_p3)
writedlm("final_e3_985.txt",final_e3)

using DelimitedFiles
for ii = 1:10, win in [1,2,3,4,5,6,7,8]
    #win=findmax(final_p1[:,ii])[2]
    if final_p3[win,ii]>0.01
        if win == 2
            pars = readdlm("hxt3_985/pts_$(ii)_$win.txt")[:,1]
            fixed_p = readdlm("potential_particles/int_g_midpoint_parameters_985.txt")[:,1]
            pars = 10 .^ [fixed_p[1:29];pars[1:5];10.0;0.0;pars[6:7];10.0;pars[8];0.0;pars[9];fixed_p[43:48];fixed_p[49:52]]#mth1,mig1
            pars[35] = -10^10
        elseif win == 1 
        ...
        writedlm("best_fit_parameters_each_model_each_run_985/hxt_3_best_particle_model_$(win)_run_$(ii).txt",pars)
    end
end