...
def hxt4model(t, y, p, glucose, *args):
    g = glucose(t, *args)
    hxt4 = y[0]
    mig1 = y[1]
    mig2 = y[2]
    ...
    snf1 = (
        ...
        * (1 + std1 / p.ksnf1std1) ** p.nsnf1
        / ((1 + std1 / p.ksnf1std1) ** p.nsnf1 + p.ell * (1 + g / p.ksnf1) ** p.msnf1)
    )
    # Hxt4
    dydt[0] = (
        p.shxt4
        ...
        - p.dhxt4 * hxt4
        - p.dhxt4g * hxt4 / (1 + (g / p.kdhxt4) ** p.mdhxt4)
    )
    # Mig1
    ...
    dydt[3] = (
        ...
        * mth1
        * g**p.mmth1snf3
        / (p.k3**p.mmth1snf3 + g**p.mmth1snf3)
        - p.dmth1rgt2
        * mth1
        * g**p.mmth1rgt2
        / (p.k2**p.mmth1rgt2 + g**p.mmth1rgt2)
    )
    # Std1
    dydt[4] = (
        ...
        * std1
        * g**p.mstd1snf3
        / (p.k3**p.mstd1snf3 + g**p.mstd1snf3)
        - p.estd1rgt2
        * std1
        * g**p.mstd1rgt2
        / (p.k2**p.mstd1rgt2 + g**p.mstd1rgt2)
    )
    return dydt
